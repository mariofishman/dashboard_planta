...
| E02 | Deterministic | OT + location + article | OT open, recipe requirement, immutable opening quantity presence | OT opening; kilograms | Fixture proven; ES-05 blocks production use |
| E03 | Deterministic + physical | Previous OT + current OT + location + article | Closing, opening, intervening movement, tolerance | Consecutive OT boundary; kilograms | Fixture proven; ES-05 blocks production use |
| E04 | Deterministic + physical | OT + article + screw | Opening, additions, ending, total consumption, recipe and tolerance | OT closure/latest container event; kilograms and fractions | Fixture proven; ES-05 blocks production use |
...
No active A–E rule is excluded at the Phase 1 fixture level: all 21 reproduce triggered, clear, and insufficient outcomes. This does not enable all 21 for production. Rules with pending mappings or models remain disabled until their later implementation phase and Phase 10 integration tests pass.

Removed C03, C04, and C05 are intentionally absent because `docs/alert_catalog.md` does not define them as active rules.
