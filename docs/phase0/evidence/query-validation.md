...
**Production status:** not approved; Aurora MySQL staging validation remains ES-01/ES-02
...
- A05 movement logic uses the current serial warehouse's `id_equipo` compared with the source work order equipment. EmusaSoft must confirm this as the production machine-location signal under ES-01.
- A02 uses `fecha_creacion` as the candidate dispatch timestamp. ES-01 must confirm its operational meaning and the reserved-OT exclusions on staging.
- Local backup work intentionally has no replica-lag requirement. Staging and production must fail closed when the ES-02 freshness signal is unavailable or too old.
