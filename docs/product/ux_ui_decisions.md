...
This document is the canonical record of UX and UI decisions within the current product defined by `docs/product/product_definition.md`. It consolidates decisions that were previously distributed across review annotations, prototype code, product discovery, the alert catalog, and the design system.

The current product design surfaces are:

- `apps/web/` — approved Dashboard V2 local implementation;
- `prototypes/current/chat/chat-list-final.html` — conversation list;
- `prototypes/current/chat/chat-detail.html` — conversation detail and alert attachment.

This document records product behavior and presentation decisions. Business rules, alert detection, and recipient routing remain governed by `docs/product/alert_catalog.md`. Visual tokens and component rules remain governed by `docs/design/design.md`, `docs/design/brand_guidelines.md`, and `docs/design/design-system/tokens.json`. The EmusaSoft boundary remains governed by `docs/architecture/system_architecture.md`; release sequencing and wider technical scope remain governed by `docs/roadmap.md`.

For the dashboard, `apps/web/` and this document represent the current design direction. `archive/prototypes/chat-list-explorations/dashboard.html` is deprecated pre-V2 evidence and has no current product authority. For chat screens, the two current prototype files listed above and this document represent the current direction. The older `01-familiar.html`, `02-role-aware.html`, `03-operations-triage.html`, and `04-pinned-focus.html` files are exploratory alternatives, not parallel product variants.
...
Monitor complements EmusaSoft. It does not replace work-order, material-flow, production, weighing, inventory, or correction workflows. Alerts and conversations show the relevant ERP identifiers and evidence. MCP v5 exposes route templates, but direct navigation remains disabled until Phase 10 validates base URLs, authorization, compatibility, and browser behavior.
...
- The pattern has one reference date and reusable schedule types. An administrator may create any working-schedule name, such as `Tempranito`, `Mediodía`, `Tarde`, `Noche`, or `Madrugada`, and defines one start and end time for each type.
...
- Two groups may never occupy the same working schedule type on the same date anywhere in the repeating cycle. `Descanso` may be shared because it represents no work. The editor and API must reject a conflicting pattern and identify the schedule and groups that collide.
- Day-oriented schedules always occupy the upper half of a calendar date and night-oriented schedules the lower half, even when only one group is visible. A lone schedule must not stretch to fill the complete date.
- Removing a group never sends its workers to `Descanso`. Before removal, the administrator must either move every affected worker to another existing group or create a named replacement group for them. The reassignment choice is shown in a compact confirmation dialog.
- After the reference date and an explicit `Agregar grupo` action, the compact pattern editor presents one unified matrix with one row per group. Group quantity is not edited as a number: groups are added with the action and removed through the delete control in each row. Its columns are group name, `Horario inicial al [fecha de referencia]`, schedule range, consecutive days, and group removal. The consecutive-days field is narrow and centers its number. Each row represents one distinct phase in the repeating cycle, so no hidden or unassigned schedule may continue affecting the calendar. Working ranges contain start and end time in one cell and use the 24-hour `HH:mm` format; `Descanso` shows a checked `No trabaja` control without an additional container or working hours. More than one row may be `Descanso`, because each is a separate no-work phase. The editor does not separate group names, schedule definitions, and initial assignments into different sections.
...
Work-order identifiers remain compact and selectable. MCP-discovered routes remain disabled until Phase 10 validates base URLs, authorization, compatibility, and browser behavior.
