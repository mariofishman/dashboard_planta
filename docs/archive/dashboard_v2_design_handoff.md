...
**Final status:** accepted by the user as a good first version on 2026-07-22. This completed record is archived for evidence and is no longer active design authority. Current reusable design rules live in `docs/design/design.md`; Dashboard behavior lives in `docs/ux_ui_decisions.md`; non-UI production questions live in `docs/monitor_architecture_and_production_roadmap.md`.
...
2. `docs/monitor_architecture_and_production_roadmap.md` — Phase 4A scope and gates;
3. `docs/ux_ui_decisions.md` — shared Monitor UX/UI rules and dashboard behavior;
4. `docs/design/design.md`, `docs/design/brand_guidelines.md`, and `docs/design/design-system/tokens.json` — visual system;
5. `prototype/chat-list-review/dashboard.html`, `dashboard.css`, and `dashboard.js` — preferred prototype baseline;
...
The visual direction should be closer to `prototype/chat-list-review/dashboard.html` than to the current Phase 4 React dashboard, but the final implementation must be more compact and incorporate the accepted reference feedback below.
...
This no-horizontal-scroll decision is newer than the older dashboard table guidance in `docs/ux_ui_decisions.md` and supersedes it for Dashboard V2.
...
- Keep `docs/design/design.md` and `docs/ux_ui_decisions.md` unchanged during active iteration unless an existing contradiction must be resolved to continue safely.
...
2. consolidate Monitor-specific dashboard behavior and UX decisions into `docs/ux_ui_decisions.md`;
...
- The requested behavior of opening a work order directly in EmusaSoft remains unresolved. `docs/ux_ui_decisions.md` explicitly states that EmusaSoft currently provides no supported frontend route for direct navigation. Implementing the link requires an approved route contract; Dashboard V2 must not guess one.
...
- The persistent density contract is intentionally separate from the canonical design documents during active Dashboard V2 iteration. After explicit approval, its reusable rules will be consolidated into `docs/design/design.md` and its Monitor-specific behavior into `docs/ux_ui_decisions.md` as previously requested.
...
- Monitor-specific chart, filter, search, custom-range, incident-list, detail, export, responsive, and accessibility behavior was promoted to `docs/ux_ui_decisions.md`.
- The database-polling persistence concern and contextual-explanation mechanism were classified as architecture and product decisions and moved to `docs/monitor_architecture_and_production_roadmap.md`.
