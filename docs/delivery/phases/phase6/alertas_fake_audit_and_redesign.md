...
| Open | A02 includes “Después del umbral: llegó sin recepción digital” and a synthetic physical-arrival state. EmusaSoft cannot know that material physically arrived when no destination receipt was recorded. | Replace this with “Despachado hace más de 30 minutos, sin recepción registrada.” A02 must use the recorded warehouse-dispatch time and absence of destination receipt, treat physical location as unknown, and never base routing on an invented physical-arrival state. Update the implementation, interface, tests, audit, and requirement matrix together after the review. |
