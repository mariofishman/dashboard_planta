...
export interface D03LaboratoryInput {
  ...
  activeOtherAlertCodes?: string[];
}

export type D03EvaluationStatus = "clear" | "triggered" | "insufficient";
...
const cloneInput = (input: D03LaboratoryInput): D03LaboratoryInput => ({
  ...
  ...(input.activeOtherAlertCodes ? { activeOtherAlertCodes: [...input.activeOtherAlertCodes] } : {}),
});
...
export function evaluateD03(input: Partial<D03LaboratoryInput>): D03Evaluation {
  ...
  for (const field of [
    ...
    "productionReels", "wasteRecords", "containerBalanceValid",
  ] as const) {
