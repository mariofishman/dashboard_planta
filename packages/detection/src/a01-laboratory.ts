...
const consequenceTypes = new Set(["missing_dispatch", "missing_consumption", "balance"]);
...
export class A01StandaloneLaboratory {
  ...
  closeWithoutResolution(workOrderId: string, materialRequirementId: string, comment: string) {
    if (comment.trim().length === 0) throw new Error("a01_closure_comment_required");
    const requirement = this.findRequirement(workOrderId, materialRequirementId);
    const key = conditionKey(requirement);
    const open = this.latestOpen(key);
