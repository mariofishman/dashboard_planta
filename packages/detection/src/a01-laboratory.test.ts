...
describe("A01 standalone deterministic laboratory", () => {
  ...
  it("A01-13 suppresses one uninterrupted condition after administrative closure and permits recurrence", () => {
    const lab = laboratory();
    lab.poll();
    lab.closeWithoutResolution("1512001", "11", "Material was physically used outside ERP.", "admin-7");
    lab.poll();
    assert.equal(lab.snapshot().occurrences.length, 1);
    assert.equal(lab.snapshot().occurrences[0]?.closureReason, "physical_operation_outside_erp");
    assert.equal(lab.snapshot().occurrences[0]?.closureActorReference, "admin-7");
    assert.equal(lab.snapshot().occurrences[0]?.closureAt, "2026-08-01T09:00:00.000Z");
    lab.updateRequirement("1512001", "11", { reservedKg: 500 });
    lab.poll();
  ...
  it("A01-16 selects only same-OT, same-material dispatch or consumption consequences", () => {
    assert.deepEqual(selectA01CorrelatedConsequences(requirement(), [
      ...
      { id: "same-consumption", workOrderId: "1512001", materialId: "substrate-7", consequence: "missing_consumption" },
    ...
    ]), ["same-dispatch", "same-consumption"]);
  });
});
