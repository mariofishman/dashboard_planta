...
describe("D03 standalone deterministic laboratory", () => {
  ...
  it("D03-13 correlates a specific cause and suppresses a duplicate D03", () => {
    const lab = laboratory({ explainingSpecificAlertCodes: ["A06", "A06", "C01"] });
    const result = lab.poll();
    assert.equal(result.status, "correlated");
    assert.deepEqual(result.correlatedAlertCodes, ["A06"]);
    assert.equal(lab.snapshot().occurrences.length, 0);
  });
