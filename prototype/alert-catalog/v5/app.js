const families = [
  {
    code:"A", title:"Material, inventory, and production-data registration", description:"General material and output-registration controls.", alerts:[
      ...
      { id:"A04", cardChanged:true, stage:"Production declaration", title:"Possible missing or implausible production output", state:"possible", status:"Possible error", when:"Consumed input implies that more output should have been declared than the digital record currently shows.", why:"The same registration mismatch can affect reel-producing operations or sealing; only the output unit and supporting calculation change.", causes:"Delayed or incorrect output declaration, undeclared waste, incorrect consumed quantity, or an inaccurate statistical estimate.", example:"For a reel operation, estimated mass remaining exceeds rewinder capacity. For sealing, consumed reel meters and the bag specification support about 120,000 bags, but only 80,000 are represented and waste does not explain the gap.", detection:"Use one A04 incident with an operation-specific calculation. For reel output, remaining mass = consumed input − actual or estimated output − actual or estimated waste, compared with rewinder capacity. For sealing, calculate expected units from consumed meters, useful print length, repetitions, bag dimensions, and declared waste using fichas_tecnicas, produccion_trabajo_pedido_detalles, orden_trabajo_salidas, and articulo_serial. Compare expected and represented output with configurable tolerances; do not create G01." },
  ...
  },
  {
    code:"E", title:"Extrusion-specific alerts", description:"Resin inventory and formulation controls unique to extrusion and Exlam.", familyNote:{key:"e-shared-rules-v5", title:"Aggregate balance and resin proportions are separate checks", body:"D03 checks whether total input equals good output plus waste. E04 checks whether the resin mix matches the recipe and can fail even when D03 passes. Extrusion and Exlam share E04; reel declaration, handling, rate, and aggregate balance continue to use A04/A05, C06, and D03."}, alerts:[
      changedAlert("E01","Safety inventory","Required extrusion safety inventory is incomplete","upcoming","Por vencer, then Error","Three hours before an extrusion OT starts, a required resin or additive is missing from the machine-specific safety warehouse or its stock is insufficient for the next 24 hours of scheduled demand.","The affected or following extrusion OT may stop because required formulation material is not available near the machine.","The planner or supervisor did not request replenishment of the machine-specific safety warehouse on time.","At 11:00, an OT planned for 14:00 requires R-17, but the safety warehouse assigned to that machine has insufficient R-17 for scheduled demand through the next 24 hours.","Use orden_trabajo_receta_snapshot and planned extrusion OTs to calculate 24-hour demand. Compare it with resin stock in the warehouse assigned to that machine, using Warehouse and ArticleWarehouseStock or the equivalent stock query after confirming the machine-to-warehouse mapping. Do not use current OT containers for this readiness rule: each container holds one resin for the current OT, while the safety warehouse protects current and near-term orders."),
  ...
  {
    code:"G", title:"Sealing / bag making", description:"Sealing currently uses the shared alert codes.", familyNote:{key:"g-shared-rules-v5", title:"G01 is A04; G02 is removed as unverified", body:"Use A01/A02/A03 for input reels, A04 for missing or implausible bag output, A05 for output handling where applicable, A06/C02 for waste, C06 for rate, and D01/D03 for closure. G01 is removed because changing output from reels to bags does not create a new error type. G02 is removed because the ERP catalog exposes bundle counters and quantityResult on WorkOrderOutput but no independently declared bundle-total workflow was found; the 9,250-versus-10,250 example is not supported as a separate user error."}, alerts:[]
  }
];
...
function changedAlert(id, stage, title, state, status, when, why, causes, example, detection) {
  return { id, stage, title, state, status, cardChanged:true, when, why, causes, example, detection };
}
...
const reviewStorageKey = "emusa-alert-catalog-v5-review";
const reviewStates = ["pending", "approved", "commented"];

function changedSection(alert, key, label, body, className = "") {
  const changed = !alert.cardChanged && alert.changedV5?.includes(key);
