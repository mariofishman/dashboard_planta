...
> **Deprecated historical document.** This file preserves early discovery and has no current product authority. Use `docs/product/product_definition.md` for the current product definition and the active specialized documents it identifies.
...
The canonical alert catalog is `docs/product/alert_catalog.md`. Its final browser publication is `reviews/alert-catalog/publication/index.html`; annotated review iterations remain in `reviews/alert-catalog/iterations/v1/` through `reviews/alert-catalog/iterations/v10/`.
...
The lifecycle is recorded separately, including whether an incident is open, resolved, or closed without resolution. Each code's label and thresholds remain governed by `docs/product/alert_catalog.md`.
