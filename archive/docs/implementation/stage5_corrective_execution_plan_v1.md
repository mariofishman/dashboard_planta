> **Historical archive notice:** This plan was superseded on 2026-08-02 by the current [Stage 5 recovery and completion plan](../../../docs/delivery/phases/phase6/stage5_corrective_execution_plan.md). It is preserved as evidence only and has no current authority.

# Phase 6 Stage 5 checkpoint and corrective execution plan

**Artifact type:** Historical Stage 5 corrective execution plan

**Authority rule:** This file has no current authority. The active Phase 6 README and roadmap point to the current Stage 5 recovery and completion plan.
...
| 8B | Integrate passive Dashboard and Chat previews into the laboratory and complete focused human review. | Explicit human acceptance of an accurate, useful, side-effect-free integration. |
...
The approved seven-part split organizes browser acceptance by independently reviewable surface rather than repeatedly revisiting every screen for separate functional, responsive, and accessibility passes. Although seven exceeds the usual five-unit warning threshold, merging would weaken distinct result boundaries: the laboratory, Dashboard, Chat list, and Chat detail expose different identities and failure modes; reconnect is a cross-surface state-recovery boundary; and the aggregate gate must remain independent. Each surface therefore receives one complete functional, responsive, keyboard, accessibility, reduced-motion, overflow, and console-aware review where applicable.

| Sub-step | Purpose and artifact | Dependency | Exit check | Status |
| --- | --- | --- | --- | --- |
| 8.1 | Build the same-runtime browser harness, versioned browser-evidence manifest, and fail-closed identity/provenance validator. | Step 7.8 aggregate evidence and Step 5 chain-capture contract. | Laboratory, API, scheduler, Monitor database, Dashboard, and Chat share one run/experiment identity; every artifact carries exact runtime provenance; reused, stale, mock, UI-only, or different-runtime evidence is rejected. | Complete — the approved three-part sequence produced a file-verifying manifest contract, an activation-gated identity handshake derived from the active persisted experiment and exact connected runtime objects, and a real API/web diagnostic with six fail-closed substitution cases. Run `20260802-step8-runtime-review-6` is explicitly diagnostic and accepted no Step 8 surface or Stage 5 result. |
| 8.2 | Validate the connected laboratory surface, including real source controls, pending and failed-read states, experiment history, responsive layouts, keyboard operation, focus order, accessible names, reduced motion, overflow, and console behavior. | 8.1 same-runtime harness and manifest. | Durable artifacts prove the laboratory presents authoritative connected state and remains usable at 1440, 768, and 390 pixels without creating Monitor truth or accepting expected-result counters as evidence. | Complete — one connected A02 chain and the active persisted experiment are visibly tied to run `20260802-step8-browser-acceptance-1`; pending and partial-read states preserve the last committed Monitor counts; responsive, focus, drawer dismissal, accessible-name, reduced-motion-rule, overflow, and console checks pass. The artifact set remains inside the still-active aggregate runtime for 8.3–8.7. |
| 8.3 | Validate the connected Dashboard surface, including exact incident lifecycle, routing/delivery visibility, absence and resolved states, responsive layouts, keyboard operation, focus order, accessible names, reduced motion, overflow, and console behavior. | 8.1 harness and exact connected Monitor objects. | Durable artifacts reference the exact incident, routing, delivery, and cursor IDs from the runtime and match authoritative Monitor state at 1440, 768, and 390 pixels. | Complete — the same connected A02 incident was captured open, filtered to an honest zero-result state, corrected through `alertas_fake`, and captured resolved without replacing its incident, routing, delivery, conversation, or message chain. Dashboard detail, chart semantics, responsive table/card behavior, accessible lifecycle naming, reduced motion, overflow, and console checks pass. |
| 8.4 | Validate the connected Chat list surface, including authorization, exact conversation identity and ordering, cursor state, responsive layouts, keyboard operation, focus order, accessible names, reduced motion, overflow, and console behavior. | 8.1 harness and the exact conversations produced by the connected runtime. | Authorized users see the exact expected conversations and unauthorized users do not; durable artifacts reference exact conversation and cursor IDs and pass the three viewport and interaction checks. | Complete — the routed plant manager sees exactly the connected conversation with one unread alert message, while an active nonparticipant sees zero conversations and receives a fail-closed direct-detail state. A discovered implicit UI-fixture fallback and detail metadata leak were removed; demo fixtures now require explicit `?uiFixtures=1`. Responsive, filter, row-menu, focus, naming, overflow, reduced-motion, and console checks pass. |
| 8.5 | Validate the connected Chat detail surface, including exact messages, incident links, delivery/receipt state, responsive layouts, keyboard operation, focus order, accessible names, reduced motion, overflow, and console behavior. | 8.4 accepted Chat-list identities and exact downstream objects. | Durable artifacts reference the exact conversation, incident, message, delivery, receipt, and cursor IDs and prove correct content and interaction behavior at all three viewports without fixture substitution. | Complete — Chat detail renders the exact connected alert message, authoritative conversation title/participants, incident lifecycle, delivery and read receipt, message cursor, and structured A02 evidence with zero fixture messages. Alert expansion, message actions, search, responsive layout, naming, focusability, reduced motion, overflow, and console checks pass. Two connected-only metadata/lifecycle gaps were corrected and reverified. |
| 8.6 | Validate real disconnect/reconnect recovery and cross-surface continuity, deduplication, authorization, and console cleanliness. | 8.2–8.5 accepted surface behavior. | Reconnected laboratory, Dashboard, Chat list, and Chat detail recover from the exact applied cursor without missing, duplicating, leaking, or mutating authoritative objects, and no unexpected browser errors occur. | Complete — all four surfaces were reloaded through a real page/socket disconnect and returned under the identical runtime identity. The exact resolved incident, conversation, message, delivery/read state, authorization, ordering, and applied cursor remained stable; replay after cursor 3 was empty and unexpected console errors remained zero. |
| 8.7 | Add and execute one aggregate Step 8 gate that validates every run-specific browser artifact and identity, performs cleanup and protected-baseline restoration in `finally`, and publishes one accepted browser-evidence manifest. | 8.1–8.6 valid evidence producers. | One serial command passes, every required artifact belongs to the same runtime and exact committed object chain, the manifest validates, cleanup completes, and the protected baseline is independently restored. | Complete — accepted aggregate run `20260802-step8-browser-acceptance-1` contains 37 byte-verified artifacts from one runtime and exact connected chain. Its serial gate passes contract/harness tests, adversarial revalidation, contracts/conversations/API/web tests and typechecks, the web build, diff checks, manifest validation, and protected-source health in `finally`. |

#### Step 8.1 internal execution sequence

The approved three-part split separates the evidence contract, the real same-runtime identity handshake, and the connected adversarial proof. A two-part version would overload harness construction with the diagnostic and negative matrix. A four-part version would separate the schema from the validator or the connected diagnostic from its failure matrix, leaving an incomplete intermediate boundary. Three units cover the complete 8.1 exit without beginning any surface-specific acceptance assigned to 8.2–8.6.

| Sub-step | Purpose and artifact | Dependency | Exit check | Status |
| --- | --- | --- | --- | --- |
| 8.1a | Define and test the versioned browser-evidence manifest schema and fail-closed semantic identity/provenance validator. | Step 5 chain-capture contract and Step 7.8 aggregate identity. | Missing or invalid run, experiment, runtime, service, database, scheduler, artifact digest, chain reference, or exact connected-object provenance fails validation. | Complete — versioned JSON Schema and semantic/file validation require one connected runtime identity across the exact six services and four browser surfaces, run-bound service digests, exact origins and runtime endpoints, unique content/provenance paths and digests, in-window capture timestamps, exact object and Step 5 chain references, artifact-root containment, regular non-symlink files, and byte-verified content/provenance/cleanup artifacts. The first pass allowed an arbitrary identity-snapshot file to satisfy the manifest; the adversarial pass designated one required runtime identity artifact and cross-validates its parsed payload against the complete declared runtime. The later connected review also made complete cleanup mandatory for diagnostic evidence and cross-validates the cleanup file's parsed payload rather than trusting booleans and a digest. Seven focused contract tests and diff checks pass. |
| 8.1b | Build the same-runtime browser harness and runtime-identity handshake across the laboratory, API, scheduler, Monitor database, Dashboard, and Chat. | 8.1a evidence contract. | Every browser surface receives one run/experiment/runtime identity tied to the same API, scheduler owner, and Monitor database instance, and the harness cannot silently substitute another runtime. | Complete — an activation-gated development/test-only API identity endpoint now derives its response from the active persisted experiment and the exact database/runtime objects, refuses simulator or production configuration, and publishes the six required service identities and four browser targets under one run, experiment, runtime, nonce, API origin, scheduler owner, and Monitor database fingerprint. The reusable harness creates unique capture sessions, fetches the authenticated endpoint, rejects identity/origin/source/coverage drift, resolves exact surface targets, and converts the handshake into the manifest runtime. The first pass deferred service-digest verification to the manifest validator; the adversarial pass made the handshake independently recompute every service digest and cross-check the API, scheduler, and database service identities before accepting targets. Six focused harness/runtime tests and contracts/API/web typechecks pass. |
| 8.1c | Execute a connected diagnostic and adversarial failure matrix against the harness and manifest contract. | 8.1a–8.1b valid contract and identity handshake. | One diagnostic manifest validates; reused, stale, mock, UI-only, altered, or different-runtime evidence fails; cleanup completes; and no Step 8 surface or Stage 5 acceptance is claimed. | Complete — diagnostic run `20260802-step8-runtime-review-6` started one real connected API and one Vite web runtime, activated one persisted experiment-bound identity, served all four browser routes from the same web origin, and received the identical authenticated API/database/scheduler identity through that browser origin for every route. The endpoint is proven inactive before activation, unauthorized without a session, and inactive again after clearing. Its manifest contains byte-verified runtime and surface-target artifacts; its failure matrix rejects reused, stale, mock, UI-only, altered, and different-runtime evidence. Cleanup ran in `finally`, disposed the Monitor runtime, and passed protected-source health before and after. The first connected review strengthened browser-origin identity delivery and cleanup-payload validation; the second added the endpoint lifecycle denials. Final proportional validation passes: 7 evidence-contract tests, 6 harness/runtime tests, 7 contracts tests, 56 API tests, 25 web tests, the web build, all three boundary typechecks, source health, and diff checks. The ignored local diagnostic remains explicitly `accepted: false`; 8.2 surface behavior was not started. |

#### Step 8.2 internal execution sequence

The calibrated four-part split separates connected laboratory truth, failure-state behavior, complete interaction/accessibility coverage, and durable evidence publication. Two or three units would mix authoritative-state assertions with browser ergonomics or artifact integrity; five would split the three viewports into repetitive passes despite their shared surface contract. Four units cover every 8.2 exit condition once without entering Dashboard acceptance.

| Sub-step | Purpose and artifact | Dependency | Exit check | Status |
| --- | --- | --- | --- | --- |
| 8.2a | Create one connected laboratory acceptance state and capture its exact experiment, source, poll, incident, routing, conversation, message, delivery, and cursor identities. | 8.1 same-runtime harness and Step 5 chain contract. | The laboratory reads the active `test_database` experiment and exact committed Monitor chain without simulator or expected-counter substitution. | Complete — the visible laboratory shows the active experiment name and run ID, `test_database.A02.v2`, one open occurrence, one evidence, one delivery, one conversation, one alert message, and cursor 2; its artifacts carry the exact persisted object references supplied by the same-runtime session. The adversarial review found no browser-local or expected-counter substitution. |
| 8.2b | Exercise pending-poll and failed-read laboratory states against the same connected experiment. | 8.2a authoritative connected state. | Pending and failed reads remain explicit, preserve the last committed Monitor truth, and never present laboratory expectations as accepted evidence. | Complete — A03 and A05 expose their source-change-pending state, while A02 was forced through a real partial read with `missing_next_cursor`. The laboratory explicitly says the read is untrusted and Monitor keeps the last confirmed truth; the persisted 1/1 incident, evidence/delivery, and conversation/message counts remain unchanged. A following healthy poll restores `Lectura confirmada` and `Coincide`. |
| 8.2c | Validate laboratory keyboard operation, focus order, accessible names, reduced motion, console behavior, and overflow at 1440, 768, and 390 pixels. | 8.2a–8.2b connected laboratory states. | Every required interaction and viewport check passes with no unexpected console error or document-level horizontal overflow. | Complete — full-page captures at 1440×900, 768×1024, and 390×844 retain the active identity and connected revision with exact viewport-width document bounds. All enabled buttons have names, the experiment-history drawer enters the keyboard focus boundary and dismisses with Escape, the shared reduced-motion rule is present, compact controls retain their minimum visible height, and the laboratory console has zero warnings or errors. The review could not rely on a screenshot alone, so it added structured DOM, focus, and computed-rule evidence. |
| 8.2d | Publish and independently validate the laboratory artifact set and provenance. | 8.2a–8.2c accepted checks. | Durable artifacts reference the exact connected identities, cover all required laboratory states and viewports, and pass the fail-closed manifest validator. | Complete — seven laboratory artifacts are staged inside the run-specific ignored evidence root: three responsive captures, one failed-read capture, interaction/state evidence, accessibility evidence, and console evidence. Each references the same experiment and exact Monitor chain; final byte digests and provenance are intentionally produced by 8.7 after all surfaces finish so no artifact can be substituted between surface review and aggregate publication. |

#### Step 8.3 internal execution sequence

The calibrated four-part split keeps exact Dashboard object truth, lifecycle/absence behavior, responsive interaction/accessibility, and artifact closure independently reviewable. Three units would conflate state-transition proof with layout and input proof; five would duplicate the same viewport checks. Four units cover the complete Dashboard exit without entering Chat authorization.

| Sub-step | Purpose and artifact | Dependency | Exit check | Status |
| --- | --- | --- | --- | --- |
| 8.3a | Bind the connected Dashboard to the exact incident, routing decision, delivery, and change cursor created by the acceptance runtime. | 8.1 harness and 8.2 connected chain. | The visible open incident and its detail reference the exact committed object chain, with no fixture or different-runtime substitution. | Complete — Dashboard shows the connected `A02` occurrence, OT `146293.2`, machine `L03`, Laminación, its open lifecycle, and the real conversation action. The capture carries the same experiment, poll-cycle, incident, routing-decision, delivery, conversation, message, and cursor references as the laboratory. The adversarial review found the IDs are provenance rather than all visible text; the detail's exact operational facts and chain references together close that proof. |
| 8.3b | Exercise open, absent-filter, and resolved Dashboard states while preserving the same incident identity and authoritative lifecycle. | 8.3a exact open state. | Filtering can produce an honest absence state, source correction resolves the exact incident, and no duplicate incident, route, or delivery appears. | Complete — a nonmatching search produces the explicit zero-result state. The real `a02.receive` source action followed by a healthy normal poll changes the exact incident from open to resolved; a fresh Dashboard load shows `Estado: Resuelta`, while the incident count remains one and the routing/delivery chain remains unchanged. The first capture attempt retained stale search state; the improvement reloaded from the live API before accepting the resolved artifact. |
| 8.3c | Validate Dashboard chart/detail keyboard behavior, focus, names, reduced motion, console behavior, and overflow at 1440, 768, and 390 pixels. | 8.3a–8.3b authoritative states. | The chart, filters, results, detail, and navigation remain operable and accessible at all required viewports without unexpected console errors or horizontal overflow. | Complete — named lifecycle controls, semantic chart group/segments, full-row incident targets, accessible lifecycle image text, named navigation, and incident-detail controls are present. Captures at 1440×900, 768×1024, and 390×844 have no document overflow; mobile exposes one complete incident-card target and no visible table. The shared reduced-motion rule applies and the Dashboard console is clean. The review corrected an initially weak `display` check to measured visible geometry. |
| 8.3d | Publish and independently validate the Dashboard artifact set and exact provenance. | 8.3a–8.3c accepted checks. | Durable artifacts cover the required lifecycle, absence, interaction, accessibility, viewport, and console evidence and reference the exact runtime IDs. | Complete — nine Dashboard artifacts are staged: open, exact detail, absence, resolved, tablet, and mobile captures plus interaction, accessibility, and console records. Their final byte digests and provenance remain deferred only to the aggregate 8.7 finalizer inside the same still-active runtime. |

#### Step 8.4 internal execution sequence

The calibrated four-part split separates authorized identity/order proof, explicit exclusion proof, responsive keyboard/accessibility review, and artifact closure. Three units would bury the unauthorized case inside presentation checks; five would repeat the same list contract by viewport. Four units cover the Chat-list exit without beginning message-detail behavior.

| Sub-step | Purpose and artifact | Dependency | Exit check | Status |
| --- | --- | --- | --- | --- |
| 8.4a | Bind the authorized Chat list to the exact connected conversation, ordering state, open-alert metadata, and cursor. | 8.1 harness and exact conversation produced by 8.2. | The routed authorized user sees the exact connected conversation in authoritative order with its real alert and cursor-derived unread state. | Complete — the manager's list contains exactly one row keyed by the persisted conversation UUID, ordered as the sole authoritative result, with `Monitor` as the sender and one unread message tied to the connected alert-message cursor. All, unread, and pinned filters report 1, 1, and 0 respectively; the expected row survives the unread filter and the pinned filter shows an honest empty state. |
| 8.4b | Prove a separately authenticated nonparticipant cannot see or open the connected conversation. | 8.4a authorized identity. | The unauthorized Chat list excludes the conversation and direct detail access fails closed without leaking its content. | Complete after two corrections — activating machine-operator 9003 after routing creates a valid authenticated nonparticipant without adding them to the existing conversation. The first browser pass exposed eight UI-only fallback rows on an empty API result; removing implicit fallback produced `Todas 0`. Direct URL review then exposed fixture title/participant leakage and an enabled composer after 403; the second correction uses a neutral unavailable header and disables every message control. Final list and detail denial captures contain no connected or fixture content. |
| 8.4c | Validate Chat-list filters, row/menu keyboard behavior, focus, names, reduced motion, console behavior, and overflow at 1440, 768, and 390 pixels. | 8.4a–8.4b authorization boundaries. | List interaction and responsive checks pass at every viewport with no unexpected console error or horizontal overflow. | Complete — filters expose `aria-pressed`, the exact row and unread badge have explicit names, the desktop row menu opens and dismisses with Escape, the mobile menu is absent from the desktop-only affordance path, list controls and the independent scroll region remain present, and primary navigation stays visible. All three viewports match document width exactly and the console is clean; the shared reduced-motion rule applies. |
| 8.4d | Publish and independently validate the Chat-list artifact set and provenance. | 8.4a–8.4c accepted checks. | Durable artifacts cover authorization, exact identity/order/cursor, responsive interaction, accessibility, and console evidence from the same runtime. | Complete — eight Chat-list artifacts are staged: authorized desktop/tablet/mobile, unauthorized list and direct-detail captures, interaction/authorization evidence, accessibility evidence, and console evidence. Every artifact retains the same experiment, exact conversation, message, and cursor provenance for final 8.7 publication. |

#### Step 8.5 internal execution sequence

The calibrated four-part split separates exact message/receipt truth, alert-detail interactions, responsive keyboard/accessibility behavior, and artifact closure. Three units would mix persisted delivery/receipt verification with presentation actions; five would isolate individual message actions without a separate acceptance boundary. Four units cover the complete Chat-detail exit without beginning reconnect testing.

| Sub-step | Purpose and artifact | Dependency | Exit check | Status |
| --- | --- | --- | --- | --- |
| 8.5a | Bind Chat detail to the exact conversation, incident link, alert message, delivery, receipt, and cursor identities from the connected runtime. | 8.4 accepted authorized conversation. | The authorized detail loads the exact persisted alert message, records its read receipt, and exposes no fixture substitution. | Complete after one correction — direct Dashboard navigation initially showed a generic title because connected metadata depended on Chat-list browser storage. The messages contract now returns authorized title and participant metadata, with a compatible list lookup for the already-running acceptance server. Final detail shows `Gerencia de planta`, one participant, the exact message UUID/cursor, A02 incident UUID, delivery count 1, read count 1, composite receipt identity, and zero fixture messages. |
| 8.5b | Validate alert jump, expansion, work-order copy target, message menu, and conversation-search behavior against that exact message. | 8.5a exact message state. | Every tested interaction targets the same alert message and preserves authoritative incident, conversation, delivery, and receipt state. | Complete after one lifecycle correction — the open-alert jump was observed while the alert was open; after the connected source correction, the bar incorrectly continued to count the immutable alert message as open. Chat detail now resolves each alert payload's incident ID against authoritative lifecycle and refreshes on incident changes, yielding zero open alerts while retaining the historical message. The exact attachment expands its guide, exposes the named OT copy target, opens the complete message-action menu, dismisses by keyboard, and produces an honest search-empty state without mutating persisted objects. |
| 8.5c | Validate Chat-detail keyboard behavior, focus, names, reduced motion, console behavior, and overflow at 1440, 768, and 390 pixels. | 8.5a–8.5b exact content and interactions. | Message history, attachment, menus, composer, and navigation remain accessible and usable at all viewports without unexpected console errors or horizontal overflow. | Complete — the conversation header, alert region, message actions, focusable alert attachment, expansion state, search, attachment controls, composer, and navigation have explicit semantics. At 1440×900, 768×1024, and 390×844 the document width equals the viewport, the independent history and composer remain visible, and the mobile layout removes the desktop message chevron. Reduced motion is inherited from the shared system and the final authorized console is clean. |
| 8.5d | Publish and independently validate the Chat-detail artifact set and exact provenance. | 8.5a–8.5c accepted checks. | Durable artifacts cover exact content, interactions, delivery/receipt state, responsive accessibility, and console evidence from the same runtime. | Complete — seven Chat-detail artifacts are staged: desktop, expanded-alert, tablet, and mobile captures plus exact interaction/receipt, accessibility, and console records. They carry the exact incident, conversation, message, delivery, receipt, and both message/global cursor references for aggregate 8.7 validation. |

#### Step 8.6 internal execution sequence

The calibrated four-part split separates cursor/object baselining, real reload reconnects, authorization/deduplication comparison, and artifact closure. Three units would hide the distinction between reconnect mechanics and post-reconnect invariants; five would split identical surface reload logic without a new failure boundary. Four units cover the complete cross-surface recovery exit once.

| Sub-step | Purpose and artifact | Dependency | Exit check | Status |
| --- | --- | --- | --- | --- |
| 8.6a | Capture the pre-disconnect authoritative object and change-cursor baseline for all four browser surfaces. | 8.2–8.5 accepted same-runtime surfaces. | The baseline contains the exact experiment, poll, incident, route, delivery, conversation, message, receipt, and applied change identities. | Complete — the baseline records the one experiment and poll chain, resolved incident occurrence, routing/delivery/conversation/message/receipt references, global cursors 1–3, one delivered/read message, zero unread messages, and zero open alerts. |
| 8.6b | Force a real page/socket disconnect and reload reconnect on laboratory, Dashboard, Chat list, and Chat detail. | 8.6a stable baseline. | Every surface returns through the same runtime identity and reaches its connected ready/live state without fixture substitution. | Complete — each surface underwent a full page reload and Socket.IO reconnect, returned to its ready/live state, retained the exact run identity and connected data, and exposed no UI-only fixture content. |
| 8.6c | Compare post-reconnect objects, cursors, authorization, ordering, read state, and console output against the baseline. | 8.6b completed reconnects. | No object is missing, duplicated, leaked, or mutated by reconnect; applied-cursor replay is empty and unexpected console errors remain zero. | Complete — post-reconnect API comparison matches the baseline one-for-one; `GET /api/changes?after=3` returns no replay, the participant boundary and order remain stable, delivery/read counts remain 1/1, and unexpected console errors are zero. |
| 8.6d | Publish and independently validate cross-surface reconnect evidence and provenance. | 8.6a–8.6c accepted recovery checks. | Four surface reconnect artifacts plus one aggregate comparison reference the exact runtime and pass fail-closed validation. | Complete — four surface-specific reconnect records plus one aggregate comparison are staged with the exact runtime and object references. The adversarial review found that visible recovery alone could hide object drift, so the aggregate artifact adds direct API identity, object, authorization, ordering, read-state, and empty-replay checks before 8.7 finalizes byte provenance. |

#### Step 8.7 internal execution sequence

The calibrated four-part split separates acceptance semantics, serial orchestration, full execution, and adversarial revalidation. Three units would mix gate implementation with its independent negative proof; five would split closely coupled manifest and command work without a distinct result. Four small units close the aggregate boundary without repeating the surface reviews.

| Sub-step | Purpose and artifact | Dependency | Exit check | Status |
| --- | --- | --- | --- | --- |
| 8.7a | Strengthen the aggregate manifest validator and tests for mandatory surface, viewport, interaction, accessibility, console, reconnect, and exact-object coverage. | 8.1 contract and 8.2–8.6 artifact set. | An accepted aggregate cannot pass with a missing surface class, required viewport, evidence kind, clean-state assertion, or exact chain-reference type. | Complete — accepted claims must be aggregate and contain all four surfaces, screenshots at 1440/768/390, interaction/accessibility/console/reconnect evidence per surface, all nine exact object-reference classes, and the aggregate reconnect record. Parsed accessibility, console, and reconnect payloads must explicitly prove no overflow, clean output, same runtime, and stable objects. Nine focused validator tests pass. |
| 8.7b | Add one serial aggregate command that validates the finalized manifest, runs proportional regressions, and independently checks protected-source health in `finally`. | 8.7a accepted aggregate semantics. | One command owns the final validation sequence and reports restoration health even if an earlier validation fails. | Complete — `validate:phase6-stage5-step8` serializes evidence/harness tests, the finalized adversarial matrix, contracts/conversations/API/web tests and typechecks, web build, diff checks, and fresh manifest validation. It records per-command logs and always executes protected-source health in `finally`, retaining failure output when a command fails. |
| 8.7c | Finalize the current same-runtime manifest, execute the aggregate command, and preserve its run-specific evidence. | 8.7a–8.7b valid gate and 8.6 complete evidence. | The manifest, artifact bytes, provenance, cleanup record, builds, tests, typechecks, and protected-source checks all pass from the exact active run. | Complete after one serialization correction — the browser runtime completed and cleanup restored the source, but first publication correctly failed because capture-only `apiOrigin`/`attachmentId` fields and unbound viewport metadata violated the strict schema. The corrected runner normalizes these fields; the recovery finalizer reconstructed only from the already byte-bound runtime, browser results, provenance, and cleanup artifacts, added viewport-bound aggregate provenance, and published the accepted 37-artifact manifest without rerunning or substituting the browser evidence. The full serial gate then passed twice. |
| 8.7d | Run an adversarial tamper, omission, reuse, and different-runtime matrix against the finalized evidence, then independently revalidate the untouched manifest. | 8.7c accepted aggregate run. | Every invalid variant fails for the intended reason and the original accepted manifest still validates without mutation. | Complete — live-manifest clones with a missing mobile Dashboard capture, altered content digest, reused artifact paths/digests, and changed Chat-list runtime identity each fail for the intended semantic or byte-integrity reason. The untouched accepted manifest then independently revalidates with zero errors, and the final serial aggregate rerun includes this matrix. |
...
### Step 8B — Integrate passive Dashboard and Chat previews into the laboratory

#### Purpose

Let the human watch the exact connected Dashboard and Chat results beside the laboratory controls without redesigning or broadly retesting either product interface.

#### Actions

1. Add two visualization-only regions to the existing laboratory screen:
   - a passive Dashboard preview showing real committed Monitor incidents; and
   - a passive Chat preview showing the real conversations and alert messages produced for the selected observed user.
2. Show the exact applicable incident, conversation, message, delivery, receipt, and cursor IDs beside the previews so the human can correlate them with the connected run.
3. Add links that open the real Dashboard and real Chat UI in dedicated user sessions when interactive inspection is needed.
4. Read through real Monitor APIs and reuse shared display primitives where practical. Do not recreate Dashboard or Chat business rules, expected-result calculations, or a second product implementation inside the laboratory.
5. Do not mount interactive components whose effects can mutate the state being observed. The previews cannot send messages, mutate incidents or routing, mark conversations read, create delivery/read receipts, change presence, switch business state, or write expected results; those interactions remain confined to the linked real UI.
6. Prove passivity by snapshotting the applicable Monitor objects before and after preview loading, refresh, user switching, and navigation, then requiring exact equality for incident lifecycle, routing, conversations, messages, receipts, read state, presence, and committed cursors.
7. Perform one focused human review: trigger or select a connected alert, confirm that both previews show the corresponding real objects and exact IDs, follow the real-UI links, and accept or reject the usefulness and clarity of the integration.
8. Record the focused review decision and any concrete mismatch found. If a correction changes an affected surface or evidence contract, rerun that Step 8 gate and repeat the focused review.

#### Terms

- **Passive/read-only preview:** visualization that does not change the state being observed.
- **Focused human review:** confirmation that the two previews show the correct connected objects, expose useful traceability, link to the real interfaces, and introduce no observational side effects.

#### Artifacts

- unified observability workspace;
- passive-side-effect tests;
- exact-ID preview evidence; and
- focused human review record, plus correction/retest evidence only when a mismatch is found.

#### Exit

The laboratory shows accurate passive Dashboard and Chat previews from the connected runtime, all displayed IDs cross-link to the same committed objects, real-UI links work, side-effect tests pass, and the human explicitly accepts the integration.

#### Approval and Git effect

Human acceptance of the integrated previews is mandatory. It does not reopen the broader Dashboard and Chat acceptance already owned by Step 8.
...
3. Audit ledgers, chain-of-custody fields, browser artifacts, the focused human preview-review record, and reset/restore evidence.
...
- [ ] focused human preview review accepted, with any concrete mismatch corrected and retested;
...
| BROWSER-04 | Human review | Passive previews, no side effects, exact connected IDs, real links, and focused acceptance of the integration. | Absent. | None. | Missing | Build the laboratory previews and focused review record. | 8B | Side-effect tests pass and the user accepts the integration. |
...
The human observability workspace must remain passive and read-only, expose connected IDs and links to the real Dashboard and Chat, and introduce no receipts or other side effects. Human acceptance is limited to the usefulness, clarity, accuracy, and passivity of this integration; broader Dashboard and Chat acceptance remains in Step 8.
