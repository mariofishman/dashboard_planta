...
- Preserve the shared persistent floated/notched label position for every labeled form control. It must not change between empty, filled, focused, read-only, disabled, or error states. Do not override it with library automatic placement or field-specific label logic; verify text inputs, selects, autocompletes, date fields, and text areas in the browser.
